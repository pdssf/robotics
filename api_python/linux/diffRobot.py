...
def getPosition(clientID, objectHandle):    # [x,y,theta]
    """ retorna a posicao de um objeto """
    ret, position = sim.simxGetObjectPosition(
        clientID, objectHandle, -1, sim.simx_opmode_oneshot_wait)
    if (ret > 0):
...
def getDists(clientID, objectHandles):
    ...
    for i in range(7):
        x, a, b, c, d = sim.simxReadProximitySensor(
            clientID, objectHandles[i], sim.simx_opmode_buffer)
        dists.append((a, b[2]))
...
def toDeg(radians):
    """ realiza a conversão de radianos para graus """
    return radians * (180.0 / math.pi)


def movementControl(pos, goal, clientID, usensors):
    """ Realiza o controle de movimento ignorando o ângulo beta """
    ...
    for i in range(7):
        if(data[i][0] == True and data[i][1] < noDetectionDist):
            if(data[i][1] < maxDetectionDist):
                ...
                dists[i] = data[i][1]
            detect[i] = 1 - ((dists[i] - maxDetectionDist) /
...
def setTargetSpeed(clientID, phiL, phiR, revoluteJointSpeed, leftMotorHandle, rightMotorHandle, revoluteJointHandle):
    ...
    sim.simxSetJointTargetVelocity(
        clientID, rightMotorHandle, phiR, sim.simx_opmode_oneshot)
    sim.simxSetJointTargetVelocity(
        clientID, revoluteJointHandle, revoluteJointSpeed, sim.simx_opmode_oneshot)
...
def main():
    ...
    clientID = sim.simxStart(COPPELIA_SIM_IP_ADDRESS, COPPELIA_SIM_PORT,
                             True, True, 2000, 5)  # Connect to CoppeliaSim
    ...
    if (clientID != -1):
        ...
        for i in range(1, 8):
            err, usensor = sim.simxGetObjectHandle(
                clientID, 'Proximity_sensor' + str(i) + '#', sim.simx_opmode_oneshot_wait)
        ...
        a, b, image = sim.simxGetVisionSensorImage(
        ...
        while (sim.simxGetConnectionId(clientID) != -1):
            ...
            definitiveImage = cv2.cvtColor(
                np.flip(newImage, 0), cv2.COLOR_RGB2BGR)
